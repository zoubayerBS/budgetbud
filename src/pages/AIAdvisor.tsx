        ...
        <div className="space-y-12 animate-in fade-in duration-1000 p-2 md:p-4 max-w-[1400px] mx-auto pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 px-2">
                ...
                <div className="md:col-span-8 spatial-card bg-gradient-to-br from-slate-900 via-[#10141d] to-[#0a0c10] p-12 relative overflow-hidden group min-h-[500px] flex flex-col justify-between">
                    <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-500/5 rounded-full -mr-32 -mt-32 blur-[100px] group-hover:bg-indigo-500/10 transition-all duration-1000"></div>

                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-10">
                            <div className="w-2 h-2 bg-indigo-400 rounded-full animate-ping"></div>
                            <p className="text-indigo-300/50 font-black uppercase tracking-[0.2em] text-[10px]">Projection de Fin de Mois Prochain</p>
                        </div>
                            ...
                            <div className="flex items-center gap-3">
                                {analysis.isImproving ? (
                                    <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 rounded-full">
                                        ...
                                        <span className="text-emerald-400 font-black text-[10px] uppercase tracking-widest">+ {formatCurrency(analysis.avgIncome - analysis.avgExpense, currency)} / mois</span>
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-2 px-4 py-2 bg-rose-500/10 rounded-full">
                                        <TrendingDown className="w-4 h-4 text-rose-400" />
                                        <span className="text-rose-400 font-black text-[10px] uppercase tracking-widest">{formatCurrency(analysis.avgIncome - analysis.avgExpense, currency)} / mois</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="relative z-10 w-full mt-10 space-y-6">
                        <div className="flex border-t border-white/5 pt-8 gap-12">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-black text-indigo-400">{formatCurrency(analysis.avgIncome, currency)}</p>
                            </div>
                        </div>
                ...
                <div className="md:col-span-4 space-y-6">
                    <div className="bento-tile bg-white dark:bg-slate-900/40 p-8 group relative overflow-hidden border-none shadow-xl">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-slate-500 text-xs font-bold leading-relaxed">
                                Cela injecterait approx. <span className="text-indigo-500">{formatCurrency(analysis.avgExpense * 0.05, currency)}</span> de plus dans vos projets par mois.
                            </p>
                        </div>
                    </div>

                    <div className="bento-tile bg-indigo-600 p-8 group relative overflow-hidden border-none shadow-xl shadow-indigo-500/20">
                        <div className="absolute top-0 right-0 p-8 opacity-10">
                            <Target className="w-24 h-24 text-white" />
                        </div>
                        <div className="relative z-10 text-white">
                            <p className="text-[10px] font-black text-indigo-200/50 uppercase tracking-widest mb-4">Boost de Progression</p>
                            {activeGoals.length > 0 ? (
                                <>
                                    <h4 className="text-2xl font-black tracking-tighter mb-2">Objectif '{activeGoals[0].name}'</h4>
                                    <p className="text-indigo-100/60 text-xs font-bold leading-relaxed">
                                        À votre rythme actuel, vous atteindrez ce but dans environ <span className="text-white">{Math.round((activeGoals[0].target_amount - activeGoals[0].current_amount) / Math.max(1, (analysis.avgIncome - analysis.avgExpense)))} mois</span>.
                                    </p>
                                </>
                            ) : (
                                <p className="text-indigo-100/60 text-xs font-bold">Aucun objectif actif à accélérer.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {/* AI Action CTA */}
            <div className="bento-tile bg-slate-50 dark:bg-slate-900/40 p-12 text-center flex flex-col items-center gap-8 group">
                <div className="w-24 h-24 bg-white dark:bg-slate-800 rounded-[2.5rem] flex items-center justify-center text-indigo-500 shadow-2xl group-hover:rotate-12 transition-transform duration-700">
